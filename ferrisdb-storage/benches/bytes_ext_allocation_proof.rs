    ...
    group.bench_function("sequential_reads_zero_alloc", |b| {
        ...
        b.iter_batched(
            || {
                ...
                let mut reader = Cursor::new(&data);
                buf.read_exact_from(&mut reader, chunk_size * num_chunks).unwrap();
                buf.clear();
                buf
            },
            |mut buf| {
    ...
    group.bench_function("capacity_growth_tracking", |b| {
        ...
        b.iter_batched(
            ...
            |data| {
                ...
                println!("Capacity growth resulted in {} allocations", stats.allocations);
    ...
    group.bench_function("efficiency_comparison", |b| {
        ...
        b.iter_batched(
            ...
            |data| {
                ...
                buf2.read_exact_from(&mut Cursor::new(&data), MEDIUM_SIZE).unwrap();
                let ext_stats = region2.change();
                
                // Report the comparison
                println!("Standard: {} allocs, BytesMutExt: {} allocs", 
