...
**Example**: "We need to store key-value pairs in memory while allowing multiple threads to read and write safely without data corruption."
...
Rust makes thread safety explicit in the type system. You can't accidentally share mutable data between threads.
</Aside>
...
<CardGrid>
  <Card title="Arc (Shared Ownership)" icon="share">
    Like `shared_ptr` in C++ or strong references in Swift. Allows multiple parts of code to own the same data.
  </Card>
...
<Tabs>
  <TabItem label="Rust ✅">
    ...
    let data = storage.data; // Error: cannot move
    
    // Must use thread-safe methods
    storage.get(b"key"); // Safe!
    ```
  </TabItem>
  
  <TabItem label="JavaScript ⚠️">
    ```javascript
    // This "works" but has race conditions
    const storage = new Storage();
    
    // Multiple workers accessing same data = 💥
    worker1.postMessage({ storage });
    worker2.postMessage({ storage });
    ```
  </TabItem>
  
  <TabItem label="Python ⚠️">
    ```python
    # Looks safe but GIL doesn't protect everything
    storage = Storage()
    
    # Forgot the lock? Data corruption!
    storage.data[key] = value  # Race condition
    ```
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Operation Costs">
    ...
    |----------|----------|-------------|--------------|
    | Rust     | 0.05     | 0.12        | 48 bytes     |
    | Go       | 0.08     | 0.15        | 64 bytes     |
    | Java     | 0.10     | 0.18        | 96 bytes     |
    | Python   | 0.50     | 0.80        | 128 bytes    |
    | Node.js  | 0.15     | 0.25        | 80 bytes     |
  </TabItem>
  
  <TabItem label="Why the Differences">
    - **Rust**: Zero-cost abstractions, no GC overhead
    - **Go**: Efficient but GC pauses affect P99 latency
    - **Java**: Object overhead + GC collections
    - **Python**: Interpreter overhead + GIL contention
    - **Node.js**: V8 is fast but still has GC
  </TabItem>
</Tabs>
...
Rust code takes longer to write initially, but you spend less time debugging mysterious crashes at 3 AM.
</Aside>
...
You don't need to master all of Rust to be productive. Start with what you need, and the compiler will guide you.
</Aside>