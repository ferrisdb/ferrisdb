...
Without [this concept], your database would [specific bad outcome]. This is why [PostgreSQL/MySQL/Redis] spends significant effort on this.
</Aside>
...
<Aside type="tip">
The key insight: [Core concept in one sentence]
</Aside>
...
- **[Feature]**: Planned for after we understand [prerequisite]
- **[Optimization]**: Will add once we measure actual bottlenecks
</Aside>
...
<Tabs>
  <TabItem label="Our Benchmarks">
    ```bash
    # Run these yourself
    cargo bench [component]_operations
    ```
    
    Current results on M1 MacBook:
    - Operation X: [time]
    - Operation Y: [time]
  </TabItem>
...
The goal isn't to build the fastest database—it's to understand how databases work. Start simple, measure, then optimize.
</Aside>